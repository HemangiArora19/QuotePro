...
//   useEffect(() => {
//    if (user) {
//    setCurrentUser(JSON.parse(user));// NO JSON.parse
// // NO JSON.parse
//   }
...
export default function ViewEditSearch() {
  ...
  useEffect(() => {
    if (user) {
      setCurrentUser(user); 
      console.log("Current User set in ViewEditSearch:", JSON.stringify(user));
    }
  }, [user]);

  // Fetch quotations when currentUser is set
  useEffect(() => {
    if (currentUser) {
      fetchQuotations();
    }
  ...
  const fetchQuotations = async () => {
    ...
    title: 'Error',
     icon: 'warning',
    text: 'Failed to fetch quotations. Please try again. If the issue persists, please log in again.',
   })
    navigator("/login");
      console.error('Error fetching quotations:', error);
      setLoading(false);
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await api.delete(`/offer/deleteById/${id}`);
          Swal.fire('Deleted!', 'Quotation has been deleted.', 'success');
          setQuotations(quotations.filter(q => q._id !== id));
        } catch (error) {
          console.error('Error deleting quotation:', error);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-md p-6 mb-6">
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <button 
              ...
              className="px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg hover:from-blue-700 hover:to-purple-700 transition shadow-lg font-semibold whitespace-nowrap"
            >
              + New Quotation
            </button>
          </div>
        ...
        {loading ? (
          ...
          <div className="grid gap-4">
            {filteredQuotations.length > 0 ? (
              filteredQuotations.map((quotation) => (
                <div key={quotation._id} className="bg-white rounded-xl shadow-md hover:shadow-xl transition p-6">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex-1">
                      <h3 className="text-xl font-bold text-gray-900 mb-2">{quotation?.quoteNumber}</h3>
                      <p className="text-gray-600 mb-1">
              ...
              <div className="p-8">
                <div className="border-b-4 border-blue-600 pb-6 mb-6">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="text-right">
                      <div className="px-4 py-2 bg-blue-100 text-blue-700 rounded-lg font-bold mb-2">QUOTATION</div>
                      <p className="text-sm text-gray-600"><span className="font-semibold">Quote #:</span> {selectedQuotation.quoteNumber}</p>
                      <p className="text-sm text-gray-600"><span className="font-semibold">Date:</span> {selectedQuotation.quoteDate}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex justify-end mb-6">
                  <div className="w-80">
                    <div className="flex justify-between text-2xl font-bold text-gray-800 pt-3 border-t-2 border-gray-800">
                      <span>Total:</span>
                      <span className="text-blue-600">${(selectedQuotation.subtotal + selectedQuotation.taxAmount).toLocaleString()}</span>
                    </div>
                  </div>
                </div>
