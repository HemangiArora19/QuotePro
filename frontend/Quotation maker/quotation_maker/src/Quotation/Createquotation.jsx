...
/>
            ...
            {items.map((item, index) => (
  <div key={index} className="border border-gray-200 rounded-xl p-4 mb-4 bg-gray-50 shadow-sm">
    ...
    <div className="flex gap-3 mb-3">
      <div className="flex-1">
        ...
        <textarea
          ...
          placeholder="Enter service or product"
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition text-sm leading-relaxed resize-y"
        />
      </div>
      <button
        onClick={() => removeItem(index)}
        className="self-start mt-6 px-3 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition font-bold text-lg"
      >
        ×
      </button>
    </div>

    {/* Qty, Rate, Discount, Amount, Delivery */}
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
      ...
      <div>
        ...
        <input
          ...
          value={`₹${item.total.toFixed(2)}`}
          readOnly
          className="w-full px-3 py-2 border border-gray-200 rounded-lg bg-white text-blue-600 font-bold"
        />
      </div>
      <div>
        ...
        <input
          type="text"
          value={item.delievery}
          onChange={(e) => handleItemChange(index, "delievery", e.target.value)}
