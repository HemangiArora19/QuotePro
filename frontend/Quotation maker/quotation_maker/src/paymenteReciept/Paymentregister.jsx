...
// const INITIAL_ROWS = [
//   { id: "PAY-1021", party: "Ravi Textiles",    date: "2026-04-20", amount: 48000, method: "NEFT", status: "Paid" },
//   { id: "PAY-1020", party: "Singh Traders",    date: "2026-04-19", amount: 12500, method: "UPI",  status: "Paid" },
//   { id: "PAY-1019", party: "Verma & Sons",     date: "2026-04-18", amount: 31200, method: "Card", status: "Partial" },
//   { id: "PAY-1018", party: "Kaur Enterprises", date: "2026-04-17", amount: 9750,  method: "Cash", status: "Pending" },
//   { id: "PAY-1017", party: "Modi Imports",     date: "2026-04-15", amount: 22000, method: "NEFT", status: "Overdue" },
// ];
...
//           <h2 className="font-semibold text-slate-800">Edit — {row.id}</h2>
...
//             <div>
//               <label className="block text-xs font-medium text-slate-500 mb-1.5">Ref No.</label>
//               <input value={form.id} readOnly className="w-full px-3 py-2 text-sm border border-slate-200 rounded-xl bg-slate-50 text-slate-400 font-mono" />
//             </div>
//             <div>
//               <label className="block text-xs font-medium text-slate-500 mb-1.5">Date</label>
//               <input type="date" value={form.date} onChange={set("date")} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-xl text-slate-700 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 outline-none" />
//             </div>
//           </div>

//           <div>
//             <label className="block text-xs font-medium text-slate-500 mb-1.5">Party Name</label>
//             <input type="text" value={form.party} onChange={set("party")} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-xl text-slate-700 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 outline-none" />
//           </div>

//           <div className="grid grid-cols-2 gap-4">
//             <div>
//               <label className="block text-xs font-medium text-slate-500 mb-1.5">Amount (₹)</label>
//               <input type="number" value={form.amount} onChange={set("amount")} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-xl text-slate-700 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 outline-none" />
//             </div>
//             <div>
//               <label className="block text-xs font-medium text-slate-500 mb-1.5">Method</label>
//               <select value={form.method} onChange={set("method")} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-xl text-slate-700 focus:border-indigo-400 outline-none bg-white">
//                 {["NEFT","UPI","Card","Cash"].map((m) => <option key={m}>{m}</option>)}
//               </select>
//             </div>
//           </div>
...
//             <select value={form.status} onChange={set("status")} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-xl text-slate-700 focus:border-indigo-400 outline-none bg-white">
...
//           <button onClick={onClose} className="px-4 py-2 text-sm font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors">Cancel</button>
//           <button onClick={() => onSave(form)} className="px-4 py-2 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl transition-colors">Save changes</button>
//         </div>
//       </div>
//     </div>
//   );
...
//               <span className="font-semibold">{row.id} ({row.party})</span>?
...
//         <div className="flex justify-end gap-3 px-6 py-4 border-t border-slate-100">
//           <button onClick={onClose} className="px-4 py-2 text-sm font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors">Cancel</button>
//           <button onClick={onConfirm} className="px-4 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 rounded-xl transition-colors">Yes, delete</button>
//         </div>
//       </div>
//     </div>
//   );
...
//       (!s || r.party.toLowerCase().includes(s) || r.id.toLowerCase().includes(s)) &&
//       (!statusF || r.status === statusF) &&
//       (!methodF || r.method === methodF)
//     );
//   }, [rows, search, statusF, methodF]);

//   const handleSave = (updated) => {
//     setRows((prev) => prev.map((r) => r.id === updated.id ? { ...updated, amount: parseInt(updated.amount) || r.amount } : r));
//     setEditRow(null);
//   };
...
//           {[
//             { label: "Total Collected", val: "₹2,84,500", sub: "↑ This month",   subCls: "text-emerald-600" },
//             { label: "Pending",         val: "₹64,200",   sub: "8 invoices",      subCls: "text-amber-500" },
//             { label: "Overdue",         val: "₹18,750",   sub: "3 accounts",      subCls: "text-red-500" },
//             { label: "Transactions",    val: filtered.length, sub: "This month",  subCls: "text-slate-400" },
//           ].map((c) => (
...
//                   {["Ref No.","Party Name","Date","Amount","Method","Status","Actions"].map((h) => (
//                     <th key={h} className="text-left px-5 py-3.5 text-xs font-medium text-slate-400 uppercase tracking-wide whitespace-nowrap">{h}</th>
//                   ))}
//                 </tr>
//               </thead>
//               <tbody>
//                 {filtered.length === 0 ? (
//                   <tr>
//                     <td colSpan={7} className="px-5 py-10 text-center text-sm text-slate-400">No payments found.</td>
//                   </tr>
//                 ) : filtered.map((r) => (
//                   <tr key={r.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors last:border-0">
//                     <td className="px-5 py-3.5 font-mono text-xs text-slate-400">{r.id}</td>
//                     <td className="px-5 py-3.5 font-medium text-slate-800">{r.party}</td>
//                     <td className="px-5 py-3.5 text-slate-500">{fmtDate(r.date)}</td>
//                     <td className="px-5 py-3.5 font-semibold text-slate-800">{fmtAmt(r.amount)}</td>
//                     <td className="px-5 py-3.5 text-slate-500">{r.method}</td>
//                     <td className="px-5 py-3.5">
//                       <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${STATUS_CLS[r.status] || ""}`}>
//                         {r.status}
//                       </span>
//                     </td>
//                     <td className="px-5 py-3.5">
...
//       {deleteRow && <DeleteModal row={deleteRow} onClose={() => setDeleteRow(null)} onConfirm={handleDelete} />}
//     </div>
//   );
// }
